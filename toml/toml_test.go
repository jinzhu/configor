...
func TestFileMode_UnmarshalText(t *testing.T) {
	for _, test := range []struct {
		...
		var mode toml.FileMode
		if err := mode.UnmarshalText([]byte(test.str)); err != nil {
			t.Errorf("unexpected error: %s", err)
		} else if mode != toml.FileMode(test.want) {
...
func TestConfig_Encode(t *testing.T) {
	...
	//var c run.Config
	//c.Coordinator.WriteTimeout = toml.Duration(time.Minute)
...
func TestEnvOverride_Builtins(t *testing.T) {
	...
	type all struct {
		Str     string        `toml:"string"`
		Dur     toml.Duration `toml:"duration"`
		Int     int           `toml:"int"`
		Int8    int8          `toml:"int8"`
	...
	var got all
	if err := toml.ApplyEnvOverrides(env, "X", &got); err != nil {
		t.Fatal(err)
	}

	exp := all{
		Str:     "a string",
		Dur:     toml.Duration(time.Minute + time.Second),
		Int:     1,
		Int8:    2,
		Int16:   3,
		Int32:   4,
